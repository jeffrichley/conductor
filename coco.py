...
for j in range(9): # p1's location
    for k in range(9): # p2's location
        ...
        p1_q_values[new_state] = [[0, 0, 0],
                                  [0, 0, 0],
                                  [0, 0, 0]]
        p1_v[new_state] = 0

        p2_q_values[new_state] = [[0, 0, 0],
                                  [0, 0, 0],
                                  [0, 0, 0]]
        p2_v[new_state] = 0
...
for t in tqdm(range(steps), mininterval=5):
    ...
    p1_q_values[current_state][p1_action][p2_action] = (1 - alpha) * q_s_a_1 + alpha*(p1_reward + gamma*p1_v[state_prime])
    p2_q_values[current_state][p1_action][p2_action] = (1 - alpha) * q_s_a_2 + alpha*(p2_reward + gamma*p2_v[state_prime])
